...
def filter_annotations_by_image_ids(annotations, images):
    image_ids = funcy.lmap(lambda image: int(image['id']), images)
    return funcy.lfilter(lambda a: int(a['image_id']) in image_ids, annotations)


def filter_images_by_annotation_ids(images, annotations):
    annotation_ids = funcy.lmap(lambda annotation: int(annotation['image_id']), annotations)
...
def main(args):
    with open(args.annotations, 'rt', encoding='UTF-8') as annotations:
        ...
        images_with_annotations = funcy.lmap(lambda annotation: int(annotation['image_id']), annotations)

        if args.having_annotations:
            # remove images without annotations
            images = funcy.lremove(lambda image: image['id'] not in images_with_annotations, images)

        if args.multi_class:
            annotation_categories = funcy.lmap(lambda annotation: int(annotation['category_id']), annotations)
            ...
            annotation_categories = funcy.lremove(
                lambda category: annotation_categories.count(category) <= 1,
            ...
            save_coco(args.train, info, licenses,
                      images=filter_images_by_annotation_ids(images, X_train.reshape(-1)),
                      annotations=X_train.reshape(-1).tolist(),
                      categories=categories)
            save_coco(args.test, info, licenses,
                      images=filter_images_by_annotation_ids(images, X_test.reshape(-1)),
                      annotations=X_test.reshape(-1).tolist(),
                      categories=categories)
            ...
            anns_train = filter_annotations_by_image_ids(annotations, X_train)
            anns_test = filter_annotations_by_image_ids(annotations, X_test)
